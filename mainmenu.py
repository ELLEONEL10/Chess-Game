...
def about_popup():
    ...
    while popup_running:
        ...
        popup.fill(WHITE)
        pygame.draw.rect(popup, WHITE, popup.get_rect(), 3)
                ...
                for label, rect in buttons.items():
                    if rect.collidepoint(mouse_pos):
                        if label == "Multiplayer":
                            ...
                            import subprocess
                            subprocess.run(["python", "main.py"])  # Replace with your actual path
                            exit()  # Exit the current script
                        elif label == "AI Game":
                            pygame.quit()  # Close the current Pygame window
                            import subprocess
                            subprocess.run(["python", "chess/main.py"])  # Replace with your actual path
                            exit()  # Exit the current script
                        elif label == "About":
                            about_popup() 
